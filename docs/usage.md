...
桌面设置窗口提供以下标签页:
- **Floating Icon（图标选择）**：选择浮窗图标（SVG，自动修复颜色），支持 `~/.cc-remote/icons/` 自定义
- **Sound（音效设置）**：选择空闲/权限音效，支持 `~/.cc-remote/sounds/` 自定义
- **IMConfig（IM 配置）**：飞书/Telegram 平台的引导式配置，查看 config.toml
- **Logs（日志）**：显示日志目录、打开目录、开关是否将日志保存到文件
- **About（关于）**：版本信息

### 8. 日志落盘

用于排查「无响应」「崩溃」等问题。在设置页 **Logs** 标签中：

- **日志目录**：`~/.cc-remote/logs/`，点击 **Open** 直接打开
- **保存日志到文件**：开关，**默认关闭**，拨动后**实时生效**（无需重启）
- 开启后，运行日志与 Rust panic 崩溃信息按天写入 `cc-remote.<YYYY-MM-DD>.log`
- 自动保留最近 **7 天**，过期文件在应用启动时清理

> 提示：控制台（stdout）输出始终保留，开关仅控制是否额外写入文件。遇到偶发问题时建议先打开开关复现，再到日志目录取证。
...
> 排查任何问题前,建议先在设置页 **Logs** 标签开启「保存日志到文件」,复现问题后到 `~/.cc-remote/logs/` 查看当天日志(含 panic 崩溃堆栈)。
